...
- `TokMonConfigStore.swift`：TokMon 配置、UI state 与 Kimi API Key / Quota 快照的本地 JSON 读写。
- `TokMonGlassStyle.swift`：主题色、玻璃态效果与动态颜色（浅色 / 深色模式）。
- `TokMonKeychain.swift`：保留的 Kimi API Key Keychain 存取辅助（当前版本由 TokMonConfigStore 通过本地 JSON 管理）。
...
4. **权限相关改动**：涉及屏幕捕获等 macOS 权限时，同步更新 `Packaging/Info.plist` 中对应的 `UsageDescription`（例如 `NSScreenCaptureUsageDescription`），并在打包后验证权限弹窗与行为。
...
- `CLAUDE.md`：Claude Code 使用说明，内容与本文档保持一致。
