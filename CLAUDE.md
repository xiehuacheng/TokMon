...
- 迁移参考：Hono + `@hono/node-server`，TypeScript 通过 `tsx/esm` 直接运行
- 壳：SwiftUI 状态栏 App（`macos-app/`）

面向用户的交付物是 `.app`；`npm run dev` 仅用于旧 API / parity 参考的本地调试。

## 常用命令

- `npm install`：安装依赖。
- `npm run dev`：启动保留的 Hono 迁移参考服务，默认端口 `3388`。
...
5. **验证与提交**：改完至少跑 `cd macos-app && swift test`、`npx tsc --noEmit` 和 `git diff --check`。涉及独立 App 的改动要重新打包启动 `.app`。UI 改动的 PR 附截图。

6. **不要自己 `git commit` / `git push`**，除非用户明确要求。本仓库的提交风格是简短祈使句，例如 `Add MCP install feedback`、`Fix broken skill cleanup`。
