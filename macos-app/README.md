...
- Claude Code、Codex、Qwen Code 和 OpenCode 本地路径默认读取用户目录下的 `~/.claude`、`~/.codex`、`~/.qwen` 和 `~/.local/share/opencode`。
  - Codex 会扫描 `~/.codex/sessions/` 下的实时 session，以及 `~/.codex/archived_sessions/` 下的归档 session。
  - Claude Code 与 Qwen Code 分别扫描 `~/.claude/projects/` 与 `~/.qwen/projects/` 下的 `.jsonl` 日志。
