...
- popover 包含 Overview、Requests、Sessions、Quota 四个页面；Quota 页面用于管理 Kimi Code API Key 并展示周额度与 5 小时滚动额度。
- popover 右上角提供刷新、复制截图、打开设置、检查更新和退出入口。
- Kimi Quota 刷新间隔可在设置窗口的 Kimi Quota 区块配置（默认 5 分钟，支持手动或 1/5/15/60 分钟）。
- Claude Code、Codex、Kimi Code、Qwen Code 和 OpenCode 本地路径默认读取：
  - Claude Code：`~/.claude/projects/`
  - Codex：`~/.codex`（递归查找其下的 `sessions/` 与 `archived_sessions/` 子目录）
  - Kimi Code：`~/.kimi-code/`
  - Qwen Code：`~/.qwen/projects/`
  - OpenCode：`~/.local/share/opencode/`（扫描该目录下的 `opencode.db` SQLite 数据库）
  - Kimi Code 扫描时会递归查找包含 `agents` 目录的 `wire.jsonl` 日志。
...
- 状态栏统计在数据变化或 popover 打开时事件驱动刷新，不依赖固定轮询间隔；菜单栏显示项可在设置窗口的 Menu Bar 区块配置（Total Tokens、Est. Cost、Requests、Kimi Weekly Quota、Kimi 5-Hour Quota）。
- 修改 Swift 代码后，如果要验证独立版 App，需要重新运行
  `bash macos-app/scripts/build-app.sh` 并重启 `release/TokMon.app`。
- 这个 `.app` 仅做了本地 ad-hoc 签名，未经过 Apple 公证，首次分发给其他机器时可能需要处理 macOS Gatekeeper 提示。
...
生成结果包括 `macos-app/release/TokMon.app`、`macos-app/release/TokMon-<version>.dmg` 和 `macos-app/release/appcast.xml`。发布 GitHub Release 时上传 DMG 与 `appcast.xml`，并在发布说明中写入 DMG 的 SHA-256。发布说明使用中文撰写。
