window.AgentMonTokMon = (() => {
function mount(root) {
  if (!root || !root.querySelector) return { destroy() {} };
  if (!window.echarts) {
    root.innerHTML = '<div class="panel empty">ECharts failed to load</div>';
    return { destroy() {} };
  }
  let destroyed = false;
  let resizeObserver = null;
  const cleanup = [];
  const $ = s => root.querySelector(s) || document.querySelector(`[data-tokmon-global] ${s}`);
  const $$ = s => {
    const local = Array.from(root.querySelectorAll(s));
    const global = Array.from(document.querySelectorAll(`[data-tokmon-global] ${s}`));
...
function setControlsMenuOpen(open) {
  const btn = $('#btnTokensControls');
  const menu = $('#tokensControlMenu');
  if (!btn || !menu) return;
...
on($('#btnTokensControls'), 'click', (e) => {
  e.stopPropagation();
  const menu = $('#tokensControlMenu');
  setControlsMenuOpen(!menu?.classList.contains('open'));
});
on(document, 'click', (e) => {
  if (!e.target.closest('.tokens-control-wrap')) setControlsMenuOpen(false);
});
on(document, 'keydown', (e) => {
  if (e.key !== 'Escape') return;
  if ($('#pricingModal')?.classList.contains('open')) { closePricingModal(); return; }
  if ($('#sourcesModal')?.classList.contains('open')) { closeSourcesModal(); return; }
...
on($('#source'), 'change', refresh);
on($('#dateFrom'), 'change', () => { clearRangeBtn(); autoInterval(); refresh(); });
on($('#dateTo'), 'change', () => { setLiveMode(false); clearRangeBtn(); autoInterval(); refresh(); });
...
function reapplyRange() {
  if (lastRangeBtn) applyRange(lastRangeBtn, { updateGranularity: false });
}

function getRangeWindow(btn, to = new Date()) {
...
function applyRange(btn, { updateGranularity = true } = {}) {
  lastRangeBtn = btn;
  const { from, to } = getRangeWindow(btn);
  if (updateGranularity) {
    if (btn.dataset.hours) setIntervalMode('hour');
    else autoInterval(from, to);
  }
  $('#dateFrom').value = fmtDateTime(from);
  $('#dateTo').value = fmtDateTime(to);
  $$('.range-btns .rbtn[data-hours], .range-btns .rbtn[data-days]').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  setLiveMode(true);
  refresh();
}

$$('.range-btns .rbtn[data-hours], .range-btns .rbtn[data-days]').forEach(btn => {
  on(btn, 'click', () => {
    applyRange(btn);
  });
});
...
function getHeroCellSize() {
  ...
  return height || lastHeroCell || 130;
}
function alignHeroDigits() {
  const CELL = getHeroCellSize();
  const digitEls = $('#heroDigits')?.querySelectorAll('.hero-digit-inner') || [];
  digitEls.forEach((el, i) => {
    const target = parseInt(prevDigits[i]);
...
on($('#btnPricing'), 'click', (e) => {
  e.stopPropagation();
  openPricingModal();
});
on($('#pricingClose'), 'click', () => { closePricingModal(); });
...
on($('#btnSources'), 'click', (e) => {
  e.stopPropagation();
  openSourcesModal();
});
on($('#sourcesClose'), 'click', () => { closeSourcesModal(); });
on($('#sourcesCancel'), 'click', () => { closeSourcesModal(); });
on($('#sourcesModal'), 'click', (e) => {
...
async function openSourcesModal() {
  ...
  try {
    ...
    window.AgentMonDirectoryPicker?.({
      input: '#claudePath',
      openButton: '#btnClaudePathOpen',
      upButton: '#btnClaudePathUp',
      root: '#claudeSourceDirPicker',
      initialPath: claude.path || '~/.claude/projects',
      showHidden: true,
    });
    window.AgentMonDirectoryPicker?.({
      input: '#codexPath',
      openButton: '#btnCodexPathOpen',
      upButton: '#btnCodexPathUp',
      root: '#codexSourceDirPicker',
      initialPath: codex.path || '~/.codex/sessions',
      showHidden: true,
    });
    setSourceStatus('');
  } catch (err) {
...
on($('#pricingSave'), 'click', () => {
  $$('#pricingConfigList .pricing-row').forEach(row => {
...
function updateHero(forceReset, options = {}) {
  ...
  if (activeSeries === 'cost') {
    ...
    rollToNumber(cents, true, options);
  } else {
    if (forceReset) { lastHeroValue = -1; prevDigits = []; lastHeroCell = 0; }
    rollToNumber(val, false, options);
  }
}
...
async function refresh() {
  ...
  if (liveMode) {
    ...
    if (lastRangeBtn) {
      const { from } = getRangeWindow(lastRangeBtn, now);
      $('#dateFrom').value = fmtDateTime(from);
    }
  }
...
function renderTablePage(data, total, rowPrefix, rowClickFn) {
  ...
  $('#breakdownTable tbody').innerHTML = page.map(r => {
    ...
    return `<tr class="clickable-row${rowCls}" data-filter-type="${clickFn.type}" data-filter-value="${escAttr(clickFn.value)}">${rowPrefix(r)}<td class="num val">${num(v)}</td><td class="num pct">${pct}%</td></tr>`;
  }).join('');
...
async function loadRecords() {
  ...
  let html = `<button data-records-page="0" ${recordsPage === 0 ? 'disabled' : ''}>&laquo;</button>`;
  html += `<button data-records-page="${recordsPage - 1}" ${recordsPage === 0 ? 'disabled' : ''}>&lt;</button>`;
  for (let i = startP; i < endP; i++) {
    html += `<button class="${i === recordsPage ? 'active' : ''}" data-records-page="${i}">${i + 1}</button>`;
  }
  html += `<button data-records-page="${recordsPage + 1}" ${recordsPage >= totalPages - 1 ? 'disabled' : ''}>&gt;</button>`;
  html += `<button data-records-page="${totalPages - 1}" ${recordsPage >= totalPages - 1 ? 'disabled' : ''}>&raquo;</button>`;
  pager.innerHTML = html;
...
on($('#recordsJump'), 'keydown', (e) => {
  ...
  on(root, 'click', (e) => {
    ...
    if (series && root.contains(series)) { selectSeries(series.dataset.series); return; }
    const row = e.target.closest('[data-filter-type]');
    if (row && root.contains(row)) { toggleFilter(row.dataset.filterType, row.dataset.filterValue); return; }
